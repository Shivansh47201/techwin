...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  try {
    const file = path.join(process.cwd(), 'backend', 'products.json')
    const arr = fs.existsSync(file) ? JSON.parse(fs.readFileSync(file, 'utf8') || '[]') : []
    const item = arr.find((p: any) => String(p.id) === String(params.id))
...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const file = path.join(process.cwd(), 'backend', 'products.json')
    const arr = fs.existsSync(file) ? JSON.parse(fs.readFileSync(file, 'utf8') || '[]') : []
    const idx = arr.findIndex((p: any) => String(p.id) === String(params.id))
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  try {
    const file = path.join(process.cwd(), 'backend', 'products.json')
    const arr = fs.existsSync(file) ? JSON.parse(fs.readFileSync(file, 'utf8') || '[]') : []
    const filtered = arr.filter((p: any) => String(p.id) !== String(params.id))
