...
export default function RequestQuoteModal({ open, onClose }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      email: form.email,
      phone: "", // modal does not have phone
    ...
    try {
      ...
      console.error("REQUEST QUOTE MODAL ERROR:", err);
      setStatus("error");
    } finally {
      setSubmitting(false);
    }
            ...
            <div className="text-center py-8">
              <div className="mx-auto w-20 h-20 rounded-full bg-[var(--color-primary)]/10 flex items-center justify-center mb-4">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none">
                  <path d="M5 13l4 4L19 7" stroke="#0b2f4a" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </div>
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <label className="flex flex-col">
                  ...
                  <input name="name" value={form.name} onChange={(e)=>setField("name", e.target.value)}
                    className="mt-1 px-3 py-2 border rounded-md focus:ring-2 focus:ring-[var(--color-primary)]"
                    placeholder="Your full name" required />
                </label>

                <label className="flex flex-col">
                  <span className="text-sm font-medium text-slate-700">Company</span>
                  <input name="company" value={form.company} onChange={(e)=>setField("company", e.target.value)}
                    className="mt-1 px-3 py-2 border rounded-md focus:ring-2 focus:ring-[var(--color-primary)]"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <label className="flex flex-col">
                  ...
                  <select name="productInterest" value={form.productInterest}
                    onChange={(e)=>setField("productInterest", e.target.value)}
                    required
                    className="mt-1 px-3 py-2 border rounded-md focus:ring-2 focus:ring-[var(--color-primary)]"
                  >
              ...
              <label className="flex flex-col">
                ...
                <textarea
                  ...
                  rows={4}
                  className="mt-1 px-3 py-2 border rounded-md focus:ring-2 focus:ring-[var(--color-primary)]"
                  placeholder="Project details, required timeline, budget (optional)..."
                />
              </label>
              ...
              {status === "error" && (
                <p className="text-sm text-red-600 mt-1">
                  ...
                  <a href="mailto:info@techwin.com" className="underline">info@techwin.com</a>.
                </p>
              )}
            </form>
          )}
